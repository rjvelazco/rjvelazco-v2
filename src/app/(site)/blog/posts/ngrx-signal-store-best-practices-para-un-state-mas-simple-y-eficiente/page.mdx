...
<a id="introduccion"></a># **NgRx Signal Store Best Practices para un state más simple y eficiente**
...
const store = signalStore(
  ...
  withComputed((store) => {
    ...
    const shippingCost = computed(() => calculateShippingCost(store.items(), store.shippingMethod()));
